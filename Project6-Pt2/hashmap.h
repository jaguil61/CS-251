...
public:
    ...
    // keyVector
    //
    // returns a vector of all the keys for a given hashtable
    //
    vector<KeyT> keyVector()
    {
        ...
        for (int i = 0; i < this->N; i++)
        {
            ...
            {
                //cout << "Keys: " << this->HT[i].Key << endl; //for debugging purposes

                keyV.push_back(this->HT[i].Key);
            }

            else // no data so ignore
                ;
        }
